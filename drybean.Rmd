...
my algorithm; the result is in Tables \@ref(tab:alg-vs-imp-1) -
\@ref(tab:alg-vs-imp-4). I also calculated the accuracy of the two models on all
features with at least 50% importance; the results are shown in Figure
\@ref(fig:important-features).
...
imp_table("rpart2", "all", "rpart2 on full set")
```

```{r alg-vs-imp-2}
imp_table("rpart2", "min", "rpart2 on minimal set")
```

```{r alg-vs-imp-3}
imp_table("treebag", "all", "treebag on full set")
```

```{r alg-vs-imp-4}
imp_table("treebag", "min", "treebag on minimal set")
```
