...
these are explicit: aspect ratio, equivalent diameter ($Ed = \sqrt{4 A / \pi}$),
roundness, and compactness. In addition, solidity is experimentally shown to be
equal to $A/C$. These relationships are verified to 9 decimal places by the
following five conditionals:
...
Consequently, an observation is completely defined by just eleven features: the
seven remaining geometric features plus the four shape factors.

However, one or more of the five derived features may still be needed by a model
to have good predictive power. Table 1 shows the accuracy of several models
using the eleven features:
