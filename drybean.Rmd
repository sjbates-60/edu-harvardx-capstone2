...
Trained four models on four sets of features to see which could most accurately
...
Four models: linear discriminant analysis, quadratic discriminant analysis, k
nearest neighbors, and bottstrap aggregated trees.

Three sets of features: $F$, $F_{math}$, and
$\{ PC1, PC2, PC3, PC4, PC5, PC6 \}$. The last one comes from feature extraction
rather than feature selection, but is included for comparison purposes.

The high accuracy of the PCA set suggests that a good model can be constructed
with fewer than 10 features (the number of mathematically independent features).
I created a way to select a set of features where each additional feature
increases the accuracy of the model as much as possible.
