...
`knn` model. The feature sets found by the algorithm improved accuracy in every
model compared to $F_{all}$. Furthermore, the largest feature set found by the
algorithm had only 7 features and all other discovered feature sets had either 4
or 5 features. This is a significant reduction in size from the 10 features in
$F_{min}$.
...
algoriththm improved the accuracy of each model and simultaneously reduced the
...
additional feature increases accuracy, the training time is on the order of
$N^2/2$, where $N$ is the number of features. For the models chosen, the actual
training time was between $4N$ and $7N$. This could still be significant if each
iteration is slow, as it was for some of the other random forest models, such as
`rf` and `Rborist`.
